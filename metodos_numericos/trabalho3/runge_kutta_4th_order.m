...
function [xi, yi] = runge_kutta_4th_order(f,x0_y0,h_xn)
  h = h_xn(1);
  xn = h_xn(2);
  x0 = x0_y0(1);
  y0 = x0_y0(2);
  ...
  while(x <= xn)
    ...
    k3 = function_handle(f)(x + h/2,y + k2*h/2);
    k4 = function_handle(f)(x + h,y + k3*h);
...
h_xn = get_step_xn;
[xi, yi] = runge_kutta_4th_order(f,x0_y0,h_xn);
