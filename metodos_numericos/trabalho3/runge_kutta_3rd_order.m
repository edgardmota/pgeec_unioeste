...
function [xi, yi] = runge_kutta_3rd_order(f,x0_y0,h_xn)
  h = h_xn(1);
  xn = h_xn(2);
  x0 = x0_y0(1);
  y0 = x0_y0(2);
  ...
  while(x <= xn)
    ...
    k3 = function_handle(f)(x + h,y - k1*h + 2*k2*h);

    y_new = y + 1/6*(k1 + 4*k2 + k3)*h;
    x += h;
    y = y_new;
  end
end
...
h_xn = get_step_xn;
[xi, yi] = runge_kutta_3rd_order(f,x0_y0,h_xn);
